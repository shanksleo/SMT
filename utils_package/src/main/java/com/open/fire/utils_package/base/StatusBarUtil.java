...
public final class StatusBarUtil {
     ...
     */
    private static final List<String> LIGHT_MODE_BLOCK_LIST = Arrays.asList("ZUI_2.3", "360OS_2.0");
     ...
     *  https://blog.csdn.net/lihexiang2012/article/details/78552049?locationNum=2&fps=1
     * @param window      需要设置的窗口
     * @param isLightMode 是否把状态栏字体及图标颜色设置为深色
     * @return boolean 成功执行返回true
     */
    ...
    private static boolean setStatusBarLightModeMIUI(Window window, boolean isLightMode) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && isSupportOriginal(checkMIUI())) {
    ...
    private static boolean isSupportOriginal(String versioncode) {
        try {
            return Long.valueOf(versioncode) > 1499875200L;
